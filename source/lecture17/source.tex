...
\begin{frame}[fragile]
\frametitle{Min/max с предикатом}
\begin{itemize}
\usemintedstyle{tango}
\item Всегда, когда стандартный алгоритм использует сравнение оператором \mintinline{cpp}|<|, у него есть версия, принимающая произвольный бинарный предикат
\pause
\usemintedstyle{lightbulb}
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
struct point {
