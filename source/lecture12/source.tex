...
\pause
\item Должна поддерживать \textit{уровни логирования}: \mintinline{cpp}|ERROR, WARNING, INFO, DEBUG, TRACE|
\pause
...
\pause
\item Лучше всего было бы удалить (\mintinline{cpp}|= delete|) оператор присваивания класса \mintinline{cpp}|Sink|
\end{itemize}
\end{frame}
...
\end{tcblisting}
\pause
\item Как сделать ещё лучше? \pause Использовать умные указатели
\end{itemize}
\end{frame}
...
\begin{itemize}
\usemintedstyle{tango}
\item Для каждого класса с виртуальными методами компилятор создаёт особый массив указателей на функции (один на класс)
\pause
\item Для каждого виртуального метода где-то в этом массиве лежит указатель на него
\pause
\item В каждый экземпляр класса компилятор добавляет указатель (недоступный программисту!) на этот массив в некое фиксированное место (обычно -- в начало класса)
\pause
\item При вызове виртуального метода сначала достаётся нужный элемент из этого массива, и потом он вызывается как функция
\pause
\item \(\Longrightarrow\) Вызов виртуального метода немного дороже, чем вызов обычного метода
\pause
\item \(\Longrightarrow\) Размер класса с виртуальными функциями на \mintinline{cpp}|sizeof(void*)| байт больше, чем кажется :)
\end{itemize}
\end{frame}
