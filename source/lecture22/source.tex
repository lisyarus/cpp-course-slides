...
\usemintedstyle{lightbulb}
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
auto factory = static_cast<IPlugin*()>(
  dlsym(library, "factory"));
...
\begin{itemize}
\usemintedstyle{tango}
\item Экспортируем только одну функцию
\pause
\item Все остальные функции работают через понятный компилятору C++ интерфейс \begin{math}\Longrightarrow\end{math} сложнее ошибиться в использовании плагина
\pause
