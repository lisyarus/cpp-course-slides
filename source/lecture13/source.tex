...
\begin{itemize}
\usemintedstyle{tango}
\item В общем случае приведение указателя на класс-наследник к указателю на базовый класс -- нетривиальная операция
\pause
\item При обычном наследовании часто базовый класс лежит в начале объекта класса-наследника, и приведение указателей не меняет адрес
\pause
\item При множественном наследовании может потребоваться сдвинуть указатель
\pause
\item При виртуальном наследовании может потребоваться прочитать из памяти адрес, по которому лежит объект базового класса
\end{itemize}
\end{frame}
...
\begin{itemize}
\usemintedstyle{tango}
\item \mintinline{cpp}|Derived*| всегда (при публичном наследовании) приводится к \mintinline{cpp}|Base*|
\pause
\item \mintinline{cpp}|Derived**| автоматически \textbf{не приводится} к \mintinline{cpp}|Base**|: для этого нужно пройтись по массиву указателей и привести каждый отдельно (это может быть нетривиальная операция)
\end{itemize}
\end{frame}
...
\begin{itemize}
\usemintedstyle{tango}
\item Пространство имён можно разбить на несколько файлов:
\usemintedstyle{lightbulb}
\begin{multicols}{2}
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
// xml/node.h
