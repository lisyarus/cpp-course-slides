...
\begin{frame}[fragile]
\frametitle{Инициализация полей значением по умолчанию}
\begin{itemize}
\usemintedstyle{lightbulb}
\item Поля структуры/класса можно инициализировать прямо при объявлении значением по умолчанию:
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
class Array {
private:
  int* data = nullptr;
...
\end{tcblisting}
\pause
\item Эта инициализация автоматически встроится в любой конструктор, который явно не инициализирует эти поля сам
\end{itemize}
\end{frame}
...
\frametitle{Инициализация полей значением по умолчанию}
\begin{itemize}
\item Поля с таким значением по умолчанию можно не инициализировать при list/designated initialization:
\usemintedstyle{lightbulb}
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
struct point {
  int id = 0;
...
// Используем вывод шаблонных параметров классов (CTAD)
...
void parallel_sum(const int* src1, const int* src2,
  int* dst, int count)
{
  // Тут хорошо бы округлить указатель до размера кеш-линии,
