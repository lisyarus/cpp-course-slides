...
\begin{itemize}
\usemintedstyle{tango}
\item В шаблонном классе в этом подходе смысла уже мало, можно заменить на шаблонную функцию:
\usemintedstyle{lightbulb}
\begin{tcblisting}{listing engine=minted, minted language=cpp, minted options={fontsize=\scriptsize}, listing only, colback=codebg, colframe=codebg, rounded corners}
template <typename T>
T& singleton() {
...
int main() {
  singleton<Settings>().loadFromFile(...);
}
\end{tcblisting}
\pause
\usemintedstyle{tango}
...
int main() {
  ...
  while (!model.finished()) {
    view.printBoard(std::cout);
    view.processInput(std::cin, std::cout);
  }
